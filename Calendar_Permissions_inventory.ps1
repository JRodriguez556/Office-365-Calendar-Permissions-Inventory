...
#>
    ...
    foreach ($MB in $MBList) {
        ...
        $calendarfolder = (Get-MailboxFolderStatistics $MB.PrimarySmtpAddress.ToString() -FolderScope Calendar | ? {$_.FolderType -eq "Calendar" -and $_.Movable -eq $false}).Identity.Replace("\",":\").Replace([char]63743,"/")
        ...
        if ($CondensedOutput) {
            ...
            foreach ($entry in $MBrights) {
                if ($entry.User.UserType.Value -eq "Internal") {
                    $internal = ("$($entry.User.ADRecipient.PrimarySmtpAddress):$($entry.AccessRights)" + ";" + $internal)
                }
                elseif ($entry.User.UserType.Value -eq "External") {
                    $external = ("$($entry.User.DisplayName.Replace("ExchangePublishedUser.",$null)):$($entry.AccessRights)" + ";" + $external)
                }
                elseif ($entry.User.UserType.Value -eq "Unknown") {
            ...
            foreach ($entry in $MBrights) {
                ...
                $varUser = "";$varType = "";
                if ($entry.User.UserType.Value -eq "Internal") { $varUser = $entry.User.ADRecipient.PrimarySmtpAddress; $varType = "Internal" }
                elseif ($entry.User.UserType.Value -eq "Default") { $varUser = $entry.User.DisplayName; $varType = "Default" }
                elseif ($entry.User.UserType.Value -eq "External") { $varUser = $entry.User.DisplayName.Replace("ExchangePublishedUser.",$null); $varType = "External" }
                elseif ($entry.User.UserType.Value -eq "Unknown") { $varUser = $entry.User.DisplayName; $varType = "Orphaned" }
                else { continue }
